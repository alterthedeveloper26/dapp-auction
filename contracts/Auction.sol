...
contract Auction {
  ...
  function Auction() public payable {
    ...
    uint256[] memory firstItemPoll;
    uint256[] memory secondItemPoll;
    items[1] = Item({ itemId: 1, itemTokens: firstItemPoll });
    items[2] = Item({ itemId: 2, itemTokens: secondItemPoll });
    //** End code here**/
  }
  ...
  function bid(uint256 _itemId, uint256 _count) public payable {
    ...
    /*
        Part 1 Task 4. Implement the three conditions below.
            ...
            4.2 If there are no tokens remaining with the bidder, revert. ---i think this is not nesscessary and we should replace it with "_count > 0" check---
    ...
    // ** Start code here. 2 lines approximately. **/
    uint256 bidderBalance = tokenDetails[msg.sender].remainingTokens;
    require(bidderBalance >= _count);
    require(bidderBalance > 0);
    require(_itemId <= 2);
    ...
    // ** Start code here. 1 line approximately. **
    uint256 balance = bidderBalance - _count;
    //** End code here. **
  ...
  function revealWinners() public onlyOwner {
    ...
    for (uint256 id = 0; id < 3; id++) {
      ...
      if (currentItem.itemTokens.length != 0) {
        ...
        uint256 randomIndex = (block.number / currentItem.itemTokens.length) %
